...
import com.yq.wx.wxpay.sdk.WXPayConstants.SignType;
 ...
 * @Author : Yanqiang
 * @Date : 2019-08-06
 * @Description : 最外层工具类，服务层调用
 *  有些功能是我自己加的，有些我改了一部分，不过差异不大
 */
public class WXPay {
    ...
    /**
     * 微信回调通知url，必须为外网可访问的url，不允许带参数，如果参数中传了notify_url，则商户平台上配置的回调地址将不会生效。
     */
     ...
     * @Author : Yanqiang
     * @Date : 2019-08-06
     * @Param : config          MyWXPayConfig 配置类
     * @Param : notifyUrl       回调接口url
     * @Param : autoReport      自动报告
     * @Param : useSandbox      沙箱环境false为正式/true为沙箱环境）
     * @Return :
     * @Description : 主要都用的这个构建
     *     PS: 建议改成都用 MD5 加密 sign，在某些功能中只支持 MD5，这个后面会介绍到
     */
    ...
    public Map<String, String> transfers(Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs) throws Exception {
        String respXml = this.requestWithCert(WXPayConstants.TRANSFERS_URL_SUFFIX, this.fillRequestDataTransfers(reqData), connectTimeoutMs, readTimeoutMs);
        System.out.println(respXml);
     ...
     * 微信提现使用
     */
    public Map<String, String> fillRequestDataTransfers(Map<String, String> reqData) throws Exception {
        reqData.put("sign", WXPayUtil.generateMD5Signature(reqData, config.getKey()));
        return reqData;
    }
}
